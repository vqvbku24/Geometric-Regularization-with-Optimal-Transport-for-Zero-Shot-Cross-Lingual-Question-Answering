...
# Usage: bash hindi/run_eval_all_hi.sh <ckpt_dir_1> <ckpt_dir_2> ...
# Example: bash hindi/run_eval_all_hi.sh checkpoint_stage2_hi/m1_vanilla_kd checkpoint_stage2_hi/m2_static_ot

if [ "$#" -eq 0 ]; then
    echo "Usage: $0 <checkpoint_dir_1> [checkpoint_dir_2] ..."
    echo "Example: $0 checkpoint_stage2_hi/m1_vanilla_kd"
    exit 1
fi
...
# Datasets
XQUAD_HI="dataset/xquad.hi.json"
XQUAD_EN="dataset/xquad.en.json"
SQUAD2="dataset/Squad2.0/dev-v2.0.json"
MLQA_HI="dataset/MLQA/test-context-hi-question-hi.json"
...
for CKPT_DIR in "$@"; do
    ...
    for i in "${!EPOCHS[@]}"; do
        ...
        if [ "$EP" = "best" ]; then
            CKPT_FILE="${CKPT_DIR}/stage2_hi_best.pt"
        else
            CKPT_FILE="${CKPT_DIR}/stage2_hi_epoch_${EP}.pt"
        fi
        ...
        (
...

            {
                ...
                # 1. XQuAD Hindi
                echo "[1] Evaluating XQuAD Hindi..."
                python hindi/phase4_evaluation/quick_eval_hi.py \
                ...
                # 4. MLQA Hindi
                echo "[4] Evaluating MLQA Hindi..."
                PRED_HI="${PRED_DIR}/preds_hi_epoch_${EP}.json"
                python hindi/generate_preds_hi.py \
                    --stage1_ckpt "$STAGE1_CKPT" \
                    --ckpt "$CKPT_FILE" \
                    --eval_file "$MLQA_HI" \
                    --output_pred_file "$PRED_HI"

                python mlqa_evaluation_v1.py "$MLQA_HI" "$PRED_HI" hi
